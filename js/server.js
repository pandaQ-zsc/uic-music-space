...
app.post('/api/users', (req, res) => {
  ...
  const generatedUid = uid || crypto.randomBytes(16).toString('hex'); // 生成16字节的随机字符串
  ...
  const sql = 'INSERT INTO student (uid, name, password) VALUES (?, ?, ?)';
  connection.query(sql, [generatedUid, name, password], (err, results) => {
    if (err) {
      console.error('插入数据失败:', err.message);
      res.status(500).json({ error: '插入失败' });
      return;
    }
    res.status(201).json({ uid: generatedUid, name, password });
  });
});
